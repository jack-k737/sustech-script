...
def submit(semester_data):
    ...
    for c_id, c_type, c_name in course_list:
        data = {
            "p_pylx": 1,
        ...
        if "成功" in req.text:
            ...
            course_list_tmp.append([c_id, c_type, c_name])
        # if any(map(lambda x: x in req.text, ["冲突", "已选", "已满"])):
        #     print(f"[\x1b[0;31m!\x1b[0m] ({c_name})因为({res})跳过", flush=True)
            
        time.sleep(1.2)  # 为了不给服务器太大压力，每次请求间隔1.2秒
    course_list.clear()
    course_list.extend(course_list_tmp)


if __name__ == '__main__':

    start = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '17:59', '%Y-%m-%d%H:%M')
    end = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '23:59', '%Y-%m-%d%H:%M')
    ...
    #喵课主逻辑
    while True:
        if not (start < datetime.datetime.now() < end):
            print(INFO + "不在喵课时间")
            time.sleep(1)
            continue
        try:
            submit(semester_info)
